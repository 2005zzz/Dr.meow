    ...
    public class AdminScheduleNotificationService : BackgroundService
    {
        ...
        private async Task RunSchedules()
        {
            ...
            if (now.Hour == 13 && now.Minute == 0)
                await NotifyAdmins("資料同步檢查", "請確認各系統資料一致性，並記錄今日同步狀態。", "Daily_1300");

            // 每週一 08:00
            if (now.DayOfWeek == DayOfWeek.Monday && now.Hour == 8 && now.Minute == 0)
                await NotifyAdmins("上週績效報告", "請檢視上週表單處理時效、熱門查詢主題與系統使用情況報告。", "Weekly_Mon_0800");

            // 每週六 02:00
            if (now.DayOfWeek == DayOfWeek.Saturday && now.Hour == 2 && now.Minute == 0)
                await NotifyAdmins("知識庫更新", "請檢查 SharePoint 文件異動、重新索引更新內容，並確認向量資料庫優化狀態。", "Weekly_Sat_0200");

            // 每月 25 日 09:00
            if (now.Day == 25 && now.Hour == 9 && now.Minute == 0)
                await NotifyAdmins("每月合規報告", "請執行文件時效性檢查、政策遵循狀態評估，並產生管理階層報告。", "Monthly_25_0900");
        }
        ...
        private async Task NotifyAdmins(string title, string message, string scheduleKey)
        {
            ...
            var todayKey = $"{scheduleKey}_{DateTime.Now:yyyyMMddHHmm}";
